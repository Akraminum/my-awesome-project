...
class JourneyManager(models.Manager):
    ...
    def retrieve(self):
        ...
        qs = qs.annotate(
            total_completed_steps=models.Count("journey_steps", filter=models.Q(journey_steps__is_completed=True))
        )
        qs = qs.annotate(
            total_incomplete_steps=models.Count("journey_steps", filter=models.Q(journey_steps__is_completed=False))
        )
        qs = qs.annotate(
            total_completed_steps_percentage=models.Case(
        ...
        qs = qs.annotate(
            total_incomplete_steps_percentage=models.Case(
                models.When(total_steps=0, then=0),
